...
void plutoPilot()
{
	...
	if(App.isArmSwitchOn()){
		...
		if(ABS(Accelerometer.getX() - initAccX)>10) {
		velX += (Accelerometer.getX() - initAccX)*0.01;
		}
		if(ABS(Accelerometer.getY() - initAccY)>10) {
			velY += (Accelerometer.getY() - initAccY)*0.01;
		}
		distX += velX*0.01;
		distY += velY*0.01;
	...
	Control.setUserLoopFrequency(10);
}
